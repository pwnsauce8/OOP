...
public class Rande {
  ...
  *         false v případě, když již mezi zobrazovanými byla */
  ...
  public Par jdouNaRande(Pozice mistoSchuzky, Presouvac chuzeNaRande){
    int xMuz = mistoSchuzky.x + zena.getSirka();
    
    Pozice pozMuz = new Pozice(xMuz, mistoSchuzky.y + yPosunMuze());
    chuzeNaRande.presunNa(pozMuz, muz);
    
    Pozice pozZena = new Pozice(mistoSchuzky.x, mistoSchuzky.y + yPosunZeny());
    chuzeNaRande.presunNa(pozZena, zena);
    
    return new Par(muz, zena, yPosunMuze(), yPosunZeny());
  }
  //pro testovaci ucely
  ...
  public void jdouDomu(Par par, Presouvac chuzeDomu){
    Pozice poz = new Pozice(domaZena.x, domaZena.y - yPosunZeny());
    chuzeDomu.presunNa(poz, par);
  ...
  public int yPosunMuze() {
    int posun = 0;
    if (zena.getVyska() > muz.getVyska()) {
      posun = zena.getVyska() - muz.getVyska();
    }
    return posun;
  }
  ...
  public int yPosunZeny() {
    if (zena.getVyska() < muz.getVyska()) {
      return muz.getVyska() - zena.getVyska();
    }
    else {
      return 0;
    }
